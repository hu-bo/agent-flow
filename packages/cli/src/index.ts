...
import { SessionManager } from '@agent-flow/core/store';
...
async function runPlayground(args: string[]): Promise<void> {
  ...
  // Resolve paths relative to this package (cli) â†?monorepo root
...
async function main(): Promise<void> {
  ...
  console.log(`agent-flow v0.1.0 â€?model: ${options.model}`);
  ...
  switch (cmd) {
    ...
    case '/compact': {
      ...
      try {
        ...
        console.log(`Compressed ${result.stats.summarizedMessageCount} messages. Token reduction: ${result.stats.originalTokenCount} â†?${result.stats.compactedTokenCount}`);
      } catch (error) {
    ...
    case '/tools': {
      ...
      if (tools.length === 0) {
        ...
        for (const tool of tools) {
          console.log(`  ${tool.name} â€?${tool.description}`);
        }
      }
      break;
    }
