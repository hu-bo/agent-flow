...
export const updateAdminModelBodySchema = createAdminModelBodySchema.partial().refine(
...
export const updateModelProfileBodySchema = createModelProfileBodySchema.omit({ profileId: true }).partial().refine(
  (value) => Object.keys(value).length > 0,
  { message: 'At least one field is required' },
);
