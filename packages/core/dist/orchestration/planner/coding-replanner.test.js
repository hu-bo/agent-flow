...
function createReplanContext(goal) {
...
describe('CodingReplanner', () => {
    it('returns recovery plan for coding requests', async () => {
        ...
        const decision = await replanner.replan(createReplanContext('fix failing tests in planner and verify regression'));
    ...
    it('returns undefined for non-coding requests', async () => {
        const replanner = new CodingReplanner();
        const plan = await replanner.replan(createReplanContext('summarize architecture and explain tradeoffs'));
