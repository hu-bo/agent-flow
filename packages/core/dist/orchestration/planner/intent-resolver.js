...
export function extractRequestMessage(request) {
...
export class PlanningIntentResolver {
    resolve(request, context, semanticStep) {
        const raw = extractRequestMessage(request);
        const lowered = normalizeWhitespace(raw).toLowerCase();
        const wantsModification = EN_CHANGE_HINTS.some((hint) => lowered.includes(hint)) || includesAny(raw, ZH_CHANGE_HINTS);
        const wantsVerification = EN_VERIFY_HINTS.some((hint) => lowered.includes(hint)) || includesAny(raw, ZH_VERIFY_HINTS);
        ...
        const isCodingTask = this.detectCodingTask(raw, lowered, request, semanticStep, codingTaskType, wantsModification, wantsVerification);
        return {
            inspectionOnly: Boolean(semanticStep) && hasInspectionVerb && !wantsModification,
            wantsModification,
            wantsVerification,
            complexityScore,
            shouldDecompose: request.strategy === 'tree' ||
    ...
    detectCodingTask(raw, lowered, request, semanticStep, codingTaskType, wantsModification, wantsVerification) {
        if (semanticStep) {
    ...
    detectCodingTaskType(raw, lowered) {
    ...
    calculateComplexityScore(rawMessage, context) {
