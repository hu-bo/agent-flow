...
function isRecoveryDecision(value: RecoveryDecision | AgentPlan): value is RecoveryDecision {
...
function planStrategyFingerprint(plan: AgentPlan): string {
...
function normalizeFailureText(error: string): string {
  return error
    .toLowerCase()
...
function failureFingerprint(error: string, step?: AgentStep): string {
  return `failure_${hashText(`${step?.kind ?? 'verification'}:${step?.toolName ?? step?.title ?? 'objective'}:${normalizeFailureText(error)}`)}`;
}

function attemptOutputs(plan: AgentPlan, outputs: Record<string, unknown>): Record<string, unknown> {
  const ids = new Set(plan.steps.map((step) => step.id));
  return Object.fromEntries(Object.entries(outputs).filter(([stepId]) => ids.has(stepId)));
}

function persistedAttempts(checkpoints: CheckpointRecord[]): AttemptSummary[] {
  return checkpoints
    .filter((checkpoint) => checkpoint.metadata.recoveryAttempt === true)
    .map((checkpoint) => checkpoint.output)
    .filter((output): output is AttemptSummary =>
      Boolean(output) && typeof output === 'object' && typeof (output as AttemptSummary).attemptId === 'string',
    );
}
...
export class DefaultPlanExecutor implements PlanExecutor {
  ...
  async *execute(
    ...
    const attempts: AttemptSummary[] = persistedAttempts(storedCheckpoints);
    ...
    let activeStrategy: RecoveryStrategy = {
      id: 'initial-strategy',
    ...
    while (true) {
      ...
      let failedStep: AgentStep | undefined;
      let failureMessage: string | undefined;
      let recoveryTrigger: 'execution_failure' | 'verification_failure' = 'execution_failure';
      try {
        ...
        latestVerification = await this.verifyCompletion(
          ...
          session,
          context,
          attemptOutputs(activePlan, outputs),
          checkpoints.slice(attemptCheckpointStart),
          events.slice(attemptEventStart),
          attemptNumber,
          emit,
        );
  ...
  private async recordAttemptCheckpoint(
    session: AgentSession,
    checkpoints: CheckpointRecord[],
